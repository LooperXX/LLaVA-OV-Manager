...
class ModelArguments:
    ...
    mm_tunable_parts: Optional[str] = field(
        default=None, metadata={"help": 'Could be "mm_mlp_adapter", "mm_manager", "mm_vision_resampler", "mm_mlp_adapter,mm_manager", "mm_vision_tower,mm_mlp_adapter,mm_language_model", "mm_mlp_adapter,mm_language_model"'}
    )
    ...
    # Manager Part
    mm_manager_type: Optional[str] = field(default=None) # static, adaptive
    mm_manager_grid_type: Optional[str] = field(default=None) # main, grid, all
    mm_manager_vision_select_layers_start: Optional[int] = field(default=-1) # 0, 13
    mm_manager_vision_select_layers_interval: Optional[int] = field(default=-1) # 1
    mm_manager_vision_select_layers_end: Optional[int] = field(default=-1) # 26
    mm_manager_injection_start: Optional[int] = field(default=-1) # 0
    mm_manager_injection_interval: Optional[int] = field(default=-1) # 2, 3, 4
    mm_manager_injection_end: Optional[int] = field(default=-1) # 24 for 0.5B, 28 for 7B
    mm_manager_softmax_temperature: Optional[float] = field(default=1.0)
    mm_manager_select_layer_index_list: Optional[str] = field(default=None)
    mm_manager_index_list: Optional[str] = field(default=None)
    mm_manager_residual: Optional[bool] = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # Manager Part
    mm_manager_lr: Optional[float] = None
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            # rank0_print(f"Image: {len(image_file)}, {len(image)}")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.mm_manager_type is not None:
        overwrite_config["mm_manager_type"] = model_args.mm_manager_type
        overwrite_config["mm_manager_grid_type"] = model_args.mm_manager_grid_type
        overwrite_config["mm_manager_vision_select_layers_start"] = model_args.mm_manager_vision_select_layers_start
        overwrite_config["mm_manager_vision_select_layers_interval"] = model_args.mm_manager_vision_select_layers_interval
        overwrite_config["mm_manager_vision_select_layers_end"] = model_args.mm_manager_vision_select_layers_end
        overwrite_config["mm_manager_injection_start"] = model_args.mm_manager_injection_start
        overwrite_config["mm_manager_injection_interval"] = model_args.mm_manager_injection_interval
        overwrite_config["mm_manager_injection_end"] = model_args.mm_manager_injection_end
        overwrite_config["mm_manager_softmax_temperature"] = model_args.mm_manager_softmax_temperature
        overwrite_config["mm_manager_residual"] = model_args.mm_manager_residual
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            elif model_args.mm_manager_type is not None:
                model = LlavaQwenManagerForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if model_args.mm_manager_type is not None:
                model.get_model().manager.requires_grad_(False)
            ...
            if "mm_manager" in tunable_parts:
                assert model_args.mm_manager_type is not None, "mm_manager_type must be specified if mm_manager is in mm_tunable_parts"
                for name, param in model.named_parameters():
                    if "manager" in name:
                        param.requires_grad_(True)
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "manager" not in name:
                        param.requires_grad_(True)
        ...
        model.config.mm_manager_lr = training_args.mm_manager_lr
