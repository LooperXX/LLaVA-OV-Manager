...
# Log Path and Rank
RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-stage1.5-manager"
HYPER_NAME="${MANAGER_VISION_SELECT_LAYERS_START}-${MANAGER_VISION_SELECT_LAYERS_INTERVAL}-${MANAGER_VISION_SELECT_LAYERS_END}-${MANAGER_INJECTION_START}-${MANAGER_INJECTION_INTERVAL}-${MANAGER_INJECTION_END}-${MANAGER_RESIDUAL}"
RUN_NAME="${RUN_NAME}-${STAGE1_MANAGER_LR}-${MANAGER_LR}-${MANAGER_TYPE}-${MANAGER_GRID_TYPE}-${HYPER_NAME}"
# RUN_NAME="${RUN_NAME}-${MANAGER_LR}-${MANAGER_TYPE}-${MANAGER_GRID_TYPE}-${HYPER_NAME}"
...
PREV_STAGE_CHECKPOINT="${PREV_STAGE_CHECKPOINT}-${STAGE1_MANAGER_LR}-${MANAGER_TYPE}-all-${HYPER_NAME}"
