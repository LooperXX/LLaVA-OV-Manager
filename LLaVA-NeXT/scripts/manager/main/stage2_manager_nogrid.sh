...
export WANDB_RUN_GROUP="stage2"
export WANDB_JOB_TYPE="train"
export PYTORCH_CUDA_ALLOC_CONF=garbage_collection_threshold:0.6,max_split_size_mb:10240
...
# Log Path and Rank
RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-stage2-fix-manager-nogrid"
HYPER_NAME="${STAGE1_MANAGER_LR}-${MANAGER_LR}-${MANAGER_TYPE}-${MANAGER_GRID_TYPE}-${MANAGER_VISION_SELECT_LAYERS_START}-${MANAGER_VISION_SELECT_LAYERS_INTERVAL}-${MANAGER_VISION_SELECT_LAYERS_END}-${MANAGER_INJECTION_START}-${MANAGER_INJECTION_INTERVAL}-${MANAGER_INJECTION_END}-${MANAGER_RESIDUAL}"
RUN_NAME="${RUN_NAME}-${HYPER_NAME}"
...
PREV_STAGE_CHECKPOINT="${CHECKPOINT_DIR}/llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-stage1.5-manager-nogrid" # replace it with your last checkpoint training from mid stage
PREV_STAGE_CHECKPOINT="${PREV_STAGE_CHECKPOINT}-${HYPER_NAME}"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --DATA_DIR ${TRAIN_CODE_DIR}/scripts/manager/main/stage2.yaml \
    --image_folder ${DATA_DIR}/stage2/images \
