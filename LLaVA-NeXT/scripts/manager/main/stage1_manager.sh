...
# Log Path and Rank
RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-stage1-manager"
HYPER_NAME="${MANAGER_LR}-${MANAGER_TYPE}-${MANAGER_GRID_TYPE}-${MANAGER_VISION_SELECT_LAYERS_START}-${MANAGER_VISION_SELECT_LAYERS_INTERVAL}-${MANAGER_VISION_SELECT_LAYERS_END}-${MANAGER_INJECTION_START}-${MANAGER_INJECTION_INTERVAL}-${MANAGER_INJECTION_END}-${MANAGER_RESIDUAL}"
...
echo "LOG_PATH: ${LOG_PATH}, RANK: ${RANK}, MODEL_SCALE: ${MODEL_SCALE}, RUN_NAME: ${RUN_NAME}, PROMPT_VERSION: ${PROMPT_VERSION}"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --mm_tunable_parts="mm_mlp_adapter,mm_manager" \
